...
with CTE_genre AS (
select movie_id, count(genre) AS num_of_genre_movie
from genre
group by movie_id
having num_of_genre_movie = 1)
Select count(movie_id) AS num_of_one_group_genre_movie from CTE_genre;
...
-- Dream Warrior Pictures or National Theatre Live both are rated 1
...
with german AS(
select sum(r.total_votes) As german_total,
...
italian AS(
select sum(r.total_votes) As italian_total,
RANK() OVER(ORDER BY SUM(r.total_votes)) AS unique_id
from movie AS m
Inner Join
ratings AS r
...
select *,
case WHEN german_total > italian_total then "Yes" else "No"
